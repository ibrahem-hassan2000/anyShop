...
function Product() {
  ...
  return (
   ...
   <div className='part1 '>
    <img className='h-[350px] md:h-[590px]' src={item.image} alt={item.title}/>
   </div>
   <div className='part2'>
    <h1 className="text-[26px] md:text-[32px]">{item.title}</h1>
    <div className='price'>
    <p className='priceBefore md:!text-[28px] !text-[22px] font-light'> {item.oldPrice}</p>
    <p className='priceAfter md:!text-[36px] !text-[28px]'>{item.price}</p>
  </div>
...
<h3>Quantity</h3>
      ...
      <Button onClick={()=>{setBaseCount(BaseCount+1)}}>
     +
    </Button>
    <p className='num'>{BaseCount}</p>
    <Button onClick={()=>{BaseCount&&setBaseCount(BaseCount-1)}}>
     -
    </Button>
      </div>
    </div>
<button className='addTo' onClick={()=>dispatch(addToCart({
   _id:item._id,
   title:item.title,
   image:item.image,
   price:item.price,
